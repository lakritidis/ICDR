...
private:
	...
	void turbo_rice_decode(unsigned int **w, unsigned int *buf,
...
/**********************
				 ...
				 * BS: block size
 */
				 void rice_coding2::turbo_rice_encode(unsigned int **w, unsigned int **buf, unsigned int bits)
				 {
					 unsigned int bp;
					 unsigned int val;
					 unsigned int i;
					 int s;
					 unsigned int out[this->block_size];

					 if (bits > 0)
					 {
						 s = ((bits * block_size)>>5);
						 for (i = 0; i < s; i++)  (*w)[i] = 0;
						 for (i = 0; i < block_size; i++)  out[i] = (*buf)[i] & MASK[bits];
				 ...
				 int rice_coding2::Compression(unsigned* input, unsigned* output, int size)
				 {
					 int i;
					 int fres;
					 unsigned int bb = 0;
	...
	//printf("b:%d, f:%d\n", b,fres);
				 ...
				 int rice_coding2::Decompression(unsigned* input, unsigned* output, int size)
				 {
					 unsigned* tmp = input;
					 int flag = (*tmp)&0x3f;
					 int b = ((*tmp)>>6);
				 ...
				 void rice_coding2::turbo_rice_decode(unsigned int **w, unsigned int *buf,
						 unsigned int bits, unsigned int flag)
